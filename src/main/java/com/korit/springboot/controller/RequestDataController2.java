...
public class RequestDataController2 {
    ...
    // POST 요청 데이터 받는 방법 2 - DTO를 통해 JSON을 데이터 받는 방법
    @PostMapping("/ewq/data2")
    ...
    // POST 요청 데이터 받는 방법 4 - 파일 데이터 받는 방법 이름을 곁들여
    // 파일 전송을 위해서는 요청 Content-Type이 꼭 multipart-form-data
    ...
    // PUT 요청 데이터 받는 방법 1 - JSON 데이터만 보통 사용  cf. JSON관련된 부분에는 @ReuquestBody가 꼭 붙는다.
